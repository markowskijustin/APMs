...
uds.3B.2022 <- read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2022-Full-Dataset.xlsx", sheet = "Table3B", skip = 0)[-1,] %>% select(BHCMISID, GrantNumber, T3b_L1_Cb, T3b_L3_Cb, T3b_L8_Ca, T3b_L5_Cb, T3b_L26_Ca, T3b_L12_Ca)
...
uds.4.2022 <- read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2022-Full-Dataset.xlsx", sheet = "Table4", skip = 0)[-1,] %>% select(BHCMISID, GrantNumber, T4_L1_Ca, T4_L2_Ca, T4_L3_Ca, T4_L4_Ca, T4_L5_Ca, T4_L16_Ca, T4_L23_Ca, T4_L7_Ca, T4_L7_Cb, T4_L8_Ca, T4_L8_Cb, T4_L9_Ca, T4_L9_Cb, T4_L10_Ca, T4_L10_Cb, T4_L11_Ca, T4_L11_Cb) 
...
uds.6a.2022 <- cbind(Year = 2022, read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2022-Full-Dataset.xlsx", sheet = "Table6A", skip = 0)) %>% rename_all(.funs = toupper) %>% filter(!is.na(BHCMISID))
...
uds.6a <- bind_rows(uds.6a.2012, uds.6a.2013, uds.6a.2014,
      uds.6a.2015, uds.6a.2016, uds.6a.2017, uds.6a.2018, uds.6a.2019, uds.6a.2020, uds.6a.2021, uds.6a.2022, uds.6a.2011andBefore) %>% 
  arrange(BHCMISID, YEAR) %>% 
...
uds.6b.2022 <- read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2022-Full-Dataset.xlsx", sheet = "Table6BClinicalmeasures", skip = 0) 
...
uds.7.2022 <- read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2022-Full-Dataset.xlsx", sheet = "Table7Clinicalmeasures", skip = 0) 
...
uds.quality.7 %>% 
  arrange(BHCMISID, Year) %>% 
  filter(Race == " Total" | is.na(Race) | (Race == "Unreported/Refused to report" & is.na(Ethnicity))) %>% 
...
## Filter for balanced panel, remove closures and late openings, TOGGLE THIS TO INCLUDE DROPOUTS
df.apm <- df.apm %>%
  left_join(df.apm %>% summarise(Counts = n(), MinYear = min(Year)), by = c("BHCMISID")) %>%  filter(Counts == length(included.years))# toggle this last bit




#  filter(Counts < 13 & MinYear == 2009)    filter(Counts == length(included.years))

## Remove some weird shit in CT
df.apm <- df.apm %>% 
  filter(BHCMISID != "01E00010") %>% 
  mutate(pap.rate = pap.rate * 100, 
         ...
         # Liquidity = (Assets - Liabilities) / Assets * 100,
         # Profitability = ngla_net_gain_loss / Assets * 100, 
         # Efficiency = ngla_revenue / Assets * 100,
         # NetWorth = (Assets - Liabilities) / Liabilities * 100,
         # Altman = 6.56 * Liquidity / 100 + 3.26 * Profitability / 100 + 6.72 * Efficiency / 100 + 1.05 * NetWorth / 100)
#%>%  filter(STATE != 'CT')
...
## Write the file
```{r}
#write_csv(df.apm, file = "/Users/jhm65/Dropbox/Projects/APMs/data_processed/analyticalfile.csv")
