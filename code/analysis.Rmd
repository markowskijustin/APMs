...
df.apm <- read_csv("//Users/jhm65/Dropbox/Git/APMs/data_processed/analyticalfile.csv") %>% 
  group_by(BHCMISID)
...
for (i in 8:14){
  ...
  df.temp$Outcome <- df.temp$Outcome * 100
    static <- did2s(
      ...
      yname = "Outcome", 
      treatment = "Tx",
        first_stage = ~ Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.None + Pct.MCD + Pct.MCR | ID + Year , 
...
df.apm.mcd <- df.apm.quality2 %>% 
  ungroup() %>% 
  left_join(df.apm %>% filter(year.rel == -1) %>% select(BHCMISID, FullParticip, Pct.MCD) %>% rename(MCD.value = Pct.MCD) %>% unique(), by = c("BHCMISID")) %>%
