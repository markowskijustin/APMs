...
df.apm %>% 
  ungroup() %>% 
  filter(Year == 2010) %>% 
    inner_join(read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2012-Full-Dataset.xlsx", sheet = "HealthCenterInfo", skip = 0) %>% 
               select(BHCMISID, UrbanRuralFlag), by = c("BHCMISID")) %>% 
  select(Class, Total:Pct.COM, UrbanRuralFlag) %>% 
...
df.apm %>% 
  ...
  left_join(rbind(read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2012-Full-Dataset.xlsx", sheet = "HealthCenterInfo", skip = 0) %>% select(BHCMISID, ReportingYear, UrbanRuralFlag), 
      read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2013-Full-Dataset.xlsx", sheet = "HealthCenterInfo", skip = 0) %>% select(BHCMISID, ReportingYear, UrbanRuralFlag),
      read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2014-Full-Dataset.xlsx", sheet = "HealthCenterInfo", skip = 0) %>% select(BHCMISID, ReportingYear, UrbanRuralFlag),
      read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2015-Full-Dataset.xlsx", sheet = "HealthCenterInfo", skip = 1) %>% rename(BHCMISID = "BHCMIS ID", ReportingYear = "Reporting Year", UrbanRuralFlag = "Urban Rural Flag") %>% select(BHCMISID, ReportingYear, UrbanRuralFlag), 
      read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2016-Full-Dataset.xlsx", sheet = "HealthCenterInfo", skip = 0) %>% rename(BHCMISID = "BHCMIS ID", ReportingYear = "Reporting Year", UrbanRuralFlag = "Urban Rural Flag") %>% select(BHCMISID, ReportingYear, UrbanRuralFlag), 
      read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2017-Full-Dataset.xlsx", sheet = "HealthCenterInfo", skip = 0) %>% rename(BHCMISID = "BHCMIS ID", ReportingYear = "Reporting Year", UrbanRuralFlag = "Urban Rural Flag") %>% select(BHCMISID, ReportingYear, UrbanRuralFlag), 
      read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2018-Full-Dataset.xlsx", sheet = "HealthCenterInfo", skip = 0) %>% rename(BHCMISID = "BHCMIS ID", ReportingYear = "Reporting Year", UrbanRuralFlag = "Urban Rural Flag") %>% select(BHCMISID, ReportingYear, UrbanRuralFlag), 
      read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2019-Full-Dataset.xlsx", sheet = "HealthCenterInfo", skip = 0) %>% rename(BHCMISID = "BHCMIS ID", ReportingYear = "Reporting Year", UrbanRuralFlag = "Urban Rural Flag") %>% select(BHCMISID, ReportingYear, UrbanRuralFlag), 
      read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2020-Full-Dataset.xlsx", sheet = "HealthCenterInfo", skip = 0) %>% rename(BHCMISID = "BHCMIS ID", ReportingYear = "Reporting Year", UrbanRuralFlag = "Urban Rural Flag") %>% select(BHCMISID, ReportingYear, UrbanRuralFlag), 
      read_excel("//Users/jhm65/Dropbox/Git/UDS/UDS-2021-Full-Dataset.xlsx", sheet = "HealthCenterInfo", skip = 0) %>% select(BHCMISID, ReportingYear, UrbanRuralFlag)) %>% mutate(ReportingYear = as.numeric(ReportingYear)), by = c("BHCMISID", "Year" = "ReportingYear")) %>% 
  select(Treatment, year.rel, Total:Pct.Homeless,UrbanRuralFlag) %>% 
  mutate(Treatment = ifelse(Treatment == 0, "Control", "Treated"), 
         Treat.YearRel = paste(Treatment, ",", year.rel)) %>% 
  select(Treat.YearRel, Total:Pct.Homeless, UrbanRuralFlag) %>% 
...
## Evaluate Missingness due to financial records

tbl_summary(df.apm %>% 
              filter(Year == 2012) %>% 
              ungroup() %>% 
              mutate(MissingFinancials = ifelse(is.na(fs_program_service_revenue), "Missing","Complete")) %>% 
              select(MissingFinancials, Pct.NHA:Pct.COM), by = MissingFinancials, 
...
ggsave(filename = "APM_Picture1.png", path = "/Users/jhm65/Dropbox/Committee/Defense Papers/Figures", plot = p.maps, width = 16, height = 9)
...
table1.totals <- table1.counts %>% 
  group_by(Treat.YearRel) %>% 
  summarise(mean(Total))



table1.counts <- cbind(Treat.YearRel = table1.counts$Treat.YearRel, table1.counts[,c(3:12)] ) %>%  #* table1.counts$Total / 100
    group_by(Treat.YearRel) %>% 
  summarise_all(mean)


t((table1.counts[,c(2:11)]) / 100 * unlist(table1.totals[,2]))
...
p.map <- plot_usmap(data = apm.history %>% 
                      rename(state = `Postal Code`) %>% 
  ungroup() %>% 
  select(Name, state, Classification) %>% 
  unique() %>% 
  arrange(Name), values = "Classification", regions = "state", color = "white", size = 0.3) + 
  #scale_fill_discrete(type = "viridis", aes(alpha = I(Treatment + 0.5))) + 
  labs(title = "By APM Class") +
...
p.map.rollout <- plot_usmap(data = apm.history %>% 
                      rename(state = `Postal Code`) %>% 
  ungroup() %>% 
  select(Name, state, Classification, Year) %>% 
    mutate(Year = (Year)) %>% 
  unique() %>% 
  arrange(Name), values = "Year", regions = "state", color = "white", size = 0.3) + 
  #scale_fill_brewer(palette="BrBG") + 
  labs(title = "By Rollout Year") +
  theme(plot.title = element_text(hjust = 0.5))
...
#p.maps <- annotate_figure(p.maps, top = text_grob("US States in Sample", face = "bold", size = 14))

p.maps

ggsave(filename = "APM_Picture1_alltime.png", path = "/Users/jhm65/Dropbox/Committee/Defense Papers/Figures", plot = p.maps, width = 16, height = 9)
```
...
df.apm.quality2 <- df.apm %>% 
  #filter(!is.na(fs_program_service_revenue)) %>% 
  ...
  #filter(att %in% c(0, 2015:2019) & Year <= 2019) %>% 
  #filter(Year <= 2019) %>% 
  select(ID, Year, Tx, Class, Class.random, year.rel, imm.rate, pap.rate, crc.rate, childbmi.rate, adultbmi.rate, htn.rate, dm.rate, Pct.NHA:Pct.COM, ACA) %>% 
...
p <- quality.class.plot + theme(panel.spacing = unit(1, "lines"),  strip.text = element_text(face = "bold", size = 12))
p
...
#ggsave(filename = "APM_Picture5.png", path = "/Users/jhm65/Dropbox/Committee/Defense Papers/Figures", plot = p, width = 16, height = 9)
```
...
## Composite Quality Plots - Pooled estimates
```{r}

## Unadjusted Trends Plot

df.apm %>% mutate(Qcomp = (crc.rate + pap.rate + childbmi.rate + adultbmi.rate + dm.rate + htn.rate) / 6) %>% 
  #filter(Tx == 1 | Treatment == 0) %>% 
  group_by(Treatment, Year) %>% 
  summarise(Qcomp = mean(Qcomp, na.rm = TRUE)) %>% 
  mutate(Treatment = ifelse(Treatment == 1, "FQHC in APM-rollout State", "FQHC in non-APM State")) %>% 
  ggplot(aes(x = Year, y = Qcomp, group = Treatment, color = Treatment)) + geom_point() + xlim(2013, 2021) + ylim(50, 70) + ylab("Quality Composite, average") + xlab("Year")
...
## Overall Effects
did2s(
      data = df.apm %>% mutate(Qcomp = (crc.rate + pap.rate + childbmi.rate + adultbmi.rate + dm.rate + htn.rate) / 6), 
      yname = "Qcomp", 
      treatment = "Tx",
        first_stage = ~ Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + ACA | ID + Year , 
        second_stage = ~ i(Tx, ref = FALSE),
        cluster_var = "ID",
      verbose = FALSE #, bootstrap = T, n_bootstraps = 500
    )
...
## Event Study
out.qual.comp <- event_study(data = df.apm %>% mutate(Qcomp = (crc.rate + pap.rate + childbmi.rate + adultbmi.rate + dm.rate + htn.rate) / 6), 
...
p.es.all.comp <- out.qual.comp[-1,] %>% 
  ...
  theme_tufte_revised()+
   theme(strip.placement = "outside") +
#theme(text=element_text(family="Times",size=12, color="black"))+
theme(panel.spacing = unit(1, "lines")) + ylab("DID Estimates") + xlab("Year Relative to APM Rollout")
...
df.apm.clincomp <- df.apm %>% 
  ...
  select(BHCMISID, ID, Year, Tx, Class, Class.Grouped, year.rel, #HIV.prev:Hypertension.care, 
         Complexity, Intensity, 
         Pct.NHA:Pct.COM, att.random, Treatment) # %>% filter(Class.Grouped %in% c(1,2))
...
f3 <- df.apm.clincomp %>% 
  ...
  group_by(Post, Treatment) %>% 
  summarise(Complexity = mean(Complexity, na.rm = TRUE), 
            #Asthma.prev = mean(Asthma.prev, na.rm = TRUE), 
            Intensity = mean(Intensity, na.rm = TRUE)) %>%
  pivot_longer(cols = c(Complexity:Intensity)) %>% 
  mutate(value = round(value, 2)) %>% 
  pivot_wider(id_cols = name, names_from = c(Treatment, Post), values_from = value) %>% 
  mutate(UnadjDID =  (`1_1` - `1_0`) - (`0_1` - `0_0`)) %>% 
  left_join(clincomp.prev, by = c("name" = "Outcome")) %>% 
  select(name, `0_0`, `0_1`, `1_0`, `1_1`, UnadjDID, Estimate, Std..Error) %>% 
  mutate(`95% CI` = paste(round(Estimate - 1.96 * Std..Error, 2), ", ", round(Estimate + 1.96 * Std..Error, 2), sep = ""), 
         `% Change` = round(Estimate / `1_0` * 100, 1)) %>% 
  select(name, `0_0`:Estimate, `95% CI`, `% Change`) %>% 
  #filter(Outcome != "2yo Immunizations") %>% 
  kable("html", booktabs = T, col.names = c("Outcome","Pre","Post","Pre","Post","Unadjusted DID","Estimate","95% CI","% Change")) %>% 
  kable_styling(latex_options = c("striped")) %>%
...
fin.class.plot <- ggplot(data=fin.class[-c(1,2,21,22),] %>% 
                               ...
                               mutate(LB = Estimate - 1.95 * Std..Error, 
                                      UB = Estimate + 1.95 * Std..Error, 
                                      Class = ifelse(Class == 2, "APM without\nQuality Incentives", "APM + Quality Incentives")), aes(y=Outcome, x=Estimate, xmin=LB, xmax=UB)) +
geom_point() + 
...
class.plot %>% 
  filter(Outcome %notin% c("Liabilities, $ millions","Medicare, pp","Commerical, pp")) %>% 
  select(Outcome, Estimate, LB, UB) %>% 
  write_clip()
...
ggsave(filename = "APM_Picture4.png", path = "/Users/jhm65/Dropbox/Committee/Defense Papers/Figures", plot = class.plot + theme(panel.spacing = unit(1, "lines"),  strip.text = element_text(face = "bold", size = 12)), width = 16, height = 9)


## Save figure as pdf for publication

ggsave(filename = "APM_Picture2.eps", path = "/Users/jhm65/Dropbox/Writing/APMs/Submission - JAMA IM/Revision", plot = class.plot + theme(panel.spacing = unit(1, "lines"),  strip.text = element_text(face = "bold", size = 12)), width = 15, height = 10)

ggsave(filename = "APM_Picture2.pdf", path = "/Users/jhm65/Dropbox/Writing/APMs/Submission - JAMA IM/Revision", plot = class.plot + theme(panel.spacing = unit(1, "lines"),  strip.text = element_text(face = "bold", size = 12)), width = 15, height = 10)
...
#adding a vertical line at the effect = 0 mark
geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) +
  facet_wrap(~V1, scales = "free_y", nrow = 3) + 
...
#ggsave(filename = "APM_PictureA3.png", path = "/Users/jhm65/Dropbox/Committee/Defense Papers/Figures", plot = out.qual.agg.composite.p, width = 16, height = 9)
#ggsave(filename = "APM_PictureA4.png", path = "/Users/jhm65/Dropbox/Committee/Defense Papers/Figures", plot = out.qual.agg.particp.mcd.p, width = 15, height = 20)
```
...
es.data.raw <- rbind(es.data.raw, cbind(i, data.frame(year.rel = as.numeric(gsub("year.rel::", "", rownames(es$coeftable))), es$coeftable) %>% 
  rbind(c(-1, 0, 0, 0, 0)) %>% 
...
out.qual <- event_study(data = df.temp, 
  yname = "Outcome", idname = "ID", tname = "Year", gname = "att", #estimator = "did2s", 
  xformla = ~Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL  + Pct.FarmWorker + Pct.Homeless )
...
p.es.all <- out.qual.es[-1,] %>% 
  ...
  filter(abs(term) <= 7) %>% 
  #cbind(Groups = c(rep("Process Quality", 60), rep("Health Outcomes", 30))) %>% 
  cbind(Groups = c(rep("Process Quality", 57*4), rep("Health Outcomes", 57*2))) %>% 
  ggplot(aes(x = term, y = estimate, color = estimator)) + geom_point(position = position_dodge(0.9)) + geom_errorbar(aes(ymin=estimate-1.96*std.error, ymax=estimate+1.96*std.error), width=.2, position = position_dodge(0.9))  + 
  facet_wrap(Groups~V1, scales = "free_y", ncol = 3, dir = "v", nrow = 3) + 
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="red", linetype="dashed", alpha=.5) + 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + ylab("DID Estimates") + xlab("Year Relative to APM Rollout")

p.es.all #+ theme(legend.position = "none")


#ggsave(filename = "APM_Picture3.png", path = "/Users/jhm65/Dropbox/Committee/Defense Papers/Figures", plot = p.es.all+ theme(legend.position = "none"), width = 16, height = 9)

## RAW data for publication
out.qual.es[-1,] %>% 
  filter(estimator %in% c("Gardner (2021)") & V1 != "2yo Immunizations") %>% 
  filter(abs(term) <= 7) %>% 
  mutate(LB = estimate-1.96*std.error, UB = estimate+1.96*std.error) %>% 
  write_clip()


es.for.pub <- out.qual.es[-1,] %>% 
  filter(estimator %in% c("Gardner (2021)") & V1 != "2yo Immunizations") %>% 
  filter(abs(term) <= 7) %>% 
  mutate(LB = estimate-1.96*std.error, UB = estimate+1.96*std.error) %>% 
  cbind(Groups = c(rep("Process Quality", 60), rep("Health Outcomes", 30))) %>% 
 ggplot(aes(x = term, y = estimate, color = estimator)) + geom_point(position = position_dodge(0.9)) + geom_errorbar(aes(ymin=estimate-1.96*std.error, ymax=estimate+1.96*std.error), width=.2, position = position_dodge(0.9))  + 
  facet_wrap(Groups~V1, scales = "free_y", ncol = 3, dir = "v", nrow = 2) + 
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="red", linetype="dashed", alpha=.5) + 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=1) + ylab("DID Estimates") + xlab("Year Relative to APM Rollout")+ theme(legend.position = "none")


ggsave(filename = "APM_Figure1.pdf", path = "/Users/jhm65/Dropbox/Writing/APMs/Submission - JAMA IM/Revision", plot = es.for.pub, width = 20, height = 15)
ggsave(filename = "APM_Figure1.eps", path = "/Users/jhm65/Dropbox/Writing/APMs/Submission - JAMA IM/Revision", plot = es.for.pub, width = 20, height = 15)


#ggsave(filename = "APM_PictureA2.png", path = "/Users/jhm65/Dropbox/Committee/Defense Papers/Figures", plot = p.es.all, width = 15, height = 20)
```
...
```{r}
p.MTC <- rbind(quality.class[-c(1,2),], fin.class) %>% 
  arrange(`Pr...t..`) %>% 
  cbind(c(1:34) / 34 * 0.1) %>% 
  mutate(Row = row_number()) %>% 
  ggplot(aes(x = Row, y = `Pr...t..`)) + geom_point() + 
  ylab("p-values") + xlab("Significance Rank") + geom_abline(slope = 1/34 * 0.1, intercept = 0, color="red", linetype="dashed", alpha=.5) + theme_minimal()




rbind(quality.all[-1,], fin.payor.prev.all) %>% 
  arrange(`Pr...t..`) %>% 
  cbind(c(1:17) / 17 * 0.1) %>% 
  mutate(Row = row_number()) %>% 
  ggplot(aes(x = Row, y = `Pr...t..`)) + geom_point() + 
  ylab("p-values") + xlab("Significance Rank") + geom_abline(slope = 1/17 * 0.1, intercept = 0, color="red", linetype="dashed", alpha=.5) + theme_minimal()

rbind(fin.payor.prev.all) %>% 
  arrange(`Pr...t..`) %>% 
  cbind(c(1:11) / 11 * 0.1) %>% 
  mutate(Row = row_number()) %>% 
  ggplot(aes(x = Row, y = `Pr...t..`)) + geom_point() + 
  ylab("p-values") + xlab("Significance Rank") + geom_abline(slope = 1/11 * 0.1, intercept = 0, color="red", linetype="dashed", alpha=.5) + theme_minimal()


#ggsave(filename = "APM_PictureA5.png", path = "/Users/jhm65/Dropbox/Committee/Defense Papers/Figures", plot = p.MTC, width = 15, height = 9)
```
...
df.trends <- df.apm %>% #filter(Treatment == 0 | as.numeric(as.character(year.rel)) %in% c(-6:-2)) %>% 
                        select(ID, Year, Tx, Treatment, Class, Class.random, year.rel, imm.rate, pap.rate, crc.rate, childbmi.rate, adultbmi.rate, htn.rate, dm.rate, Pct.NHA:Pct.COM, ACA)


pretrends <- c(0,0,0,0,0)
for (k in 10:15){
  df.temp.trends <- df.trends[,c(1:8, k, 16:27)]
    names(df.temp.trends)[9] <- 'Outcome'
model <- did2s(
      data = df.temp.trends, 
      yname = "Outcome", 
      treatment = "Tx",
        first_stage = ~ Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL  + Pct.FarmWorker + Pct.Homeless  | ID + Year , 
        second_stage = ~ i(year.rel, ref = c(-1, Inf)),
        cluster_var = "ID",
      verbose = FALSE
    )
pretrends <- rbind(pretrends, cbind(Outcome = colnames(df.trends[,k]), model$coeftable))
}
...
cbind(ordering, data.frame(pretrends))[order(ordering),] %>% 
  ...
  ylab("p-values") + xlab("Significance Rank") + geom_abline(slope = 1/30 * 0.1, intercept = 0, color="red", linetype="dashed", alpha=.5) + theme_minimal()
...
placebo <- c(0,0,0,0)
for (k in 9:18){
  df.placebo <- (df.apm %>% select(ID, Year, Tx, Treatment, Class, Class.random, year.rel, imm.rate, pap.rate, crc.rate, childbmi.rate, adultbmi.rate, htn.rate, dm.rate, Pct.NHA:Pct.COM, ACA))[,c(1:8, 16:27)]

  names(df.placebo)[k] <- 'Outcome'
  

placebo.model <- did2s(
      data = df.placebo, 
      yname = "Outcome", #Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL  + Pct.FarmWorker + Pct.Homeless 
      treatment = "Tx",
...
placebo <- rbind(placebo, placebo.model$coeftable)
print(k)
}


cbind(colnames((df.apm %>% select(ID, Year, Tx, Treatment, Class, Class.random, year.rel, imm.rate, pap.rate, crc.rate, childbmi.rate, adultbmi.rate, htn.rate, dm.rate, Pct.NHA:Pct.COM, ACA))[,c(1:8, 16:27)])[9:18], placebo[-1,]) %>% 
  data.frame()
