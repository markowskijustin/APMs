...
df.apm %>% 
  ...
  select(Treatment, year.rel, Total:Pct.COM) %>% 
  mutate(Treatment = ifelse(Treatment == 0, "Control", "Treated"), 
         Treat.YearRel = paste(Treatment, ",", year.rel)) %>% 
  select(Treat.YearRel, Total:Pct.COM) %>% 
...
as.matrix(df.apm %>% 
  ...
  select(Total:Pct.COM)) - as.matrix(df.apm %>% 
  ungroup() %>% 
...
df.apm.quality2 <- df.apm %>% 
  ...
  select(ID, Year, Tx, Class, Class.random, year.rel, imm.rate, pap.rate, crc.rate, childbmi.rate, adultbmi.rate, htn.rate, dm.rate, Pct.NHA:Pct.Homeless) %>% 
...
for (i in 8:14){
  ...
  names(df.temp)[8] <- 'Outcome'
    static <- did2s(
      ...
      yname = "Outcome", 
      treatment = "Tx",
        first_stage = ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL  | ID + Year , 
...
df.apm.quality3 <- df.apm %>% 
  ...
  #filter(att %in% c(0, 2013, 2014, 2015) & Year <= 2015) %>% 
  #filter(att %in% c(0, 2016:2021)) %>% 
    mutate(imm.prev = imm.denom / Total * 100, 
               pap.prev = pap.denom / Total * 100, 
               crc.prev = crc.denom / Total * 100, 
               childbmi.prev = childbmi.denom / Total * 100, 
               adultbmi.prev = adultbmi.denom / Total * 100, 
               htn.prev = htn.total / Total * 100,
               dm.prev = dm.total / Total * 100) %>% 
  ...
  select(ID, Year, Tx, Class, Class.random, year.rel, pap.prev:dm.prev, HIV.prev:Hypertension.prev, Pct.None:Pct.COM, ngla_revenue, ngla_expense, ngla_net_gain_loss, Assets, Liabilities, Pct.NHA:Pct.Homeless, att.random, Treatment) %>% 
  #left_join(df.apm %>% filter(year.rel == -1) %>% select(BHCMISID, Pct.NHA:Pct.COM), by = c("BHCMISID")) %>% 
  mutate(Class.Grouped = ifelse(Class %in% c("3 - Rebased per-visit rate","4 - PMPM Payment","5 - Base Rate plus Quality Bonus"), 3, 
                         ifelse(Class %in% c("1 - Medicaid Managed Care","2 - At-cost reimbursement"), 2, 
                         ifelse(Class == "0 - Control", 1, NA)))) 
...
for (i in 8:28){
  ...
  df.temp <- df.apm.quality3[,c(1:7, i, 29:ncol(df.apm.quality3))]
  names(df.temp)[8] <- 'Outcome'
    static <- did2s(
      data = df.temp, 
      yname = "Outcome", 
...
fin.payor.prev.all <- cbind(Outcome = c(
  "Cervical Cancer Screening","Colorectal Cancer Screening", "Child BMI Screening","Adult BMI Screening","Blood Pressure Control for Hypertension","A1c Control for Diabetes Mellitus",
  "HIV","Asthma","Heart Disease","Depression","Diabetes","Hypertension",
  "Pct Uninsured","Pct Medicaid","Pct Medicare","Pct Commerical",
  "Revenue","Expenses","Profit","Assets","Liabilities"), round(data.frame(fin.payor.prev.all[-1,]), 2))
...
df.apm.quality3 %>% 
  ...
  group_by(Post, Treatment) %>% 
  summarise(pap.prev = mean(pap.prev, na.rm = TRUE), 
            ...
            HIV.prev = mean(HIV.prev, na.rm = TRUE), 
            ...
            Revenue = mean(ngla_revenue, na.rm = TRUE),
  ...
  pivot_longer(cols = c(pap.prev:Liabilities)) %>% 
  mutate(value = round(value, 2)) %>% 
  pivot_wider(id_cols = name, names_from = c(Treatment, Post), values_from = value) %>% 
  mutate(UnadjDID =  (`1_1` - `1_0`) - (`0_1` - `0_0`)) %>% 
  cbind(fin.payor.prev.all) %>% 
  select(Outcome, `0_0`, `0_1`, `1_0`, `1_1`, UnadjDID, Estimate, Std..Error) %>% 
  mutate(`95% CI` = paste(round(Estimate - 1.96 * Std..Error, 2), ", ", round(Estimate + 1.96 * Std..Error, 2), sep = ""), 
         `% Change` = round(Estimate / `1_0` * 100, 1)) %>% 
  select(Outcome:Estimate, `95% CI`, `% Change`) %>% 
  ...
  kable("html", booktabs = T, col.names = c("Outcome","Pre","Post","Pre","Post","Unadjusted DID","Estimate","95% CI","% Change")) %>% 
  kable_styling(latex_options = c("striped")) %>%
  pack_rows(index = c("Quality Measure Denominator" = 6, "Condition Prevalence" = 6, "Payor Mix" = 4, "Financials" = 5)) %>% 
  add_header_above(c(" " = 1, "Control" = 2, "Treated" = 2, " " = 1, "Adjusted DID" = 3))
...
for (i in 8:23){
  for (k in 2:length(APM.class.grouped)){
  df.temp <- df.apm.quality2[,c(1:7, i, 24:ncol(df.apm.quality2))] %>% 
    #filter(Class %in% APM.class[c(1,k)])
    filter(Class.Grouped %in% APM.class.grouped[c(1, k)])
  names(df.temp)[8] <- 'Outcome'
 # df.temp$Outcome <- df.temp$Outcome * 100
...
quality.class <- cbind(Outcome = rep(c("2yo Immunizations","Cervical Cancer Screening","Colorectal Cancer Screening", "Child BMI Screening","Adult BMI Screening","Blood Pressure Control for Hypertension","A1c Control for Diabetes Mellitus","Pct Uninsured","Pct Medicaid","Pct Medicare","Pct Commerical","Revenue","Expenses","Profit","Assets","Liabilities"), each = 2), Class = rep(APM.class.grouped[-1], 16), round(data.frame(quality.class[-1,]), 2))
...
quality.class[-c(1:2),] %>% 
  ...
  add_header_above(c(" " = 1, "Cost-focused APMs" = 2, "Quality-focus APMs" = 2)) %>% 
  kable_styling(latex_options = c("striped")) %>% 
  pack_rows(index = c("Process Quality" = 4, "Health Outcomes" = 2, "Payor Mix" = 4, "Financials" = 5)) 
...
quality.class.plot <- ggplot(data=quality.class[-c(1,2),] %>% 
                               mutate(Measure = #ifelse(Outcome %in% c("Blood Pressure Control for Hypertension","A1c Control for Diabetes Mellitus"), "Quality: Outcomes", 
                                                ifelse(Outcome %in% c("Pct Uninsured","Pct Medicaid","Pct Medicare","Pct Commerical"), "Payor Mix", 
                                                ifelse(Outcome %in% c("Revenue","Expenses","Profit", "Staff","Assets","Liabilities"), "Financials", "Health Outcomes and Process Qualtiy"))) %>% 
                               mutate(Measure.f = relevel(factor(Measure), ref = "Health Outcomes and Process Qualtiy")) %>% 
...
out.qual <- did2s(
      ...
      yname = "Outcome", 
      treatment = "Tx",
        first_stage = ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.None + Pct.MCD  | ID + Year , 
        second_stage = ~ i(Tx, ref = FALSE),
        cluster_var = "ID",
      verbose = FALSE
    )
...
out.qual <- event_study(data = df.temp, 
  yname = "Outcome", idname = "ID", tname = "Year", gname = "att", estimator = "all", xformla = ~Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.None + Pct.MCD )
...
yname = "Total", idname = "ID", tname = "Year", gname = "att", estimator = "all", xformla = ~Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.None + Pct.MCD 
)
